...
[maxvertexcount(4)] void main(point VSOut input[1],
    ...
    float3 upVector = normalize(cross(rightVector, planeNormal));
    // upVector = float3(0, textureHeight, 0);

    rightVector = rightVector * halfWidth;
    upVector = upVector * halfHeight;

    // Create the billboards quad
    float3 vert[4];

    vert[0] =
        input[0].worldPos - rightVector - upVector;  // Get bottom left vertex
    vert[1] =
        input[0].worldPos + rightVector - upVector;  // Get bottom right vertex
    vert[2] =
